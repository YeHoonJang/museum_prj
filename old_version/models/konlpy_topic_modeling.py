...
def main(args):
    new_model = gensim.models.ldamodel.LdaModel.load("./model/konlpy_model.model")
    ...
    with open('/HDD/dataset/sba_museum/total_datas.csv', encoding='UTF8') as f:
        rdr = csv.reader(f)
        dict_list = list()
        for line in rdr:
            if line[1] != 'text':
    ...
    result_df.to_csv("/HDD/dataset/sba_museum/konlpy_modeling_result.csv", index=False)